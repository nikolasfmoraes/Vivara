...
def detectar_divergencias_sku(snapshots):
    """
    Compara o numero de SKUs por marca entre os snapshots disponíveis.
    Retorna lista de strings de aviso quando diferenca > 20% ou > 2000 SKUs.
    snapshots: [{"label": str, "df": DataFrame}, ...]
    """
    avisos = []
    THRESHOLD_PCT = 0.20
    THRESHOLD_ABS = 2000
    ...
    for i in range(len(snapshots)):
        for j in range(i + 1, len(snapshots)):
            ...
            marcas = sorted(set(s1["df"]["marca"].unique()) | set(s2["df"]["marca"].unique()))
            for marca in marcas:
                n1 = len(s1["df"][s1["df"]["marca"] == marca])
                n2 = len(s2["df"][s2["df"]["marca"] == marca])
                if n1 == 0 or n2 == 0:
                    continue
                diff_pct = abs(n1 - n2) / max(n1, n2)
                diff_abs = abs(n1 - n2)
                if diff_pct > THRESHOLD_PCT or diff_abs > THRESHOLD_ABS:
                    maior = s1 if n1 > n2 else s2
                    menor = s2 if n1 > n2 else s1
                    nm = n1 if n1 > n2 else n2
                    nn = n2 if n1 > n2 else n1
                    pct_str = f"{diff_pct*100:.0f}" + r"\%"
                    avisos.append(
                        f"{marca}: {maior['label']} ({nm:,} SKUs) vs "
                        f"{menor['label']} ({nn:,} SKUs) "
                        r"--- diferen\c{c}a de " + f"{diff_abs:,} SKUs ({pct_str})"
                    )
    return avisos
...
def gerar_latex(
    ...
    snapshots_raw: list[dict] | None = None,  # [{"label": str, "df": df_completo}, ...]
) -> str:
    """
    Gera um relatório LaTeX completo com os dados reais da análise.
    snapshots_raw: snapshots completos (sem filtro de inner join) para comparação de volume.
    """
    data_str = base_date.strftime("%d/%m/%Y")
    marcas = [m for m in MARCAS_COM_PANDORA if m in df_hoje["marca"].unique()]

    # ── Avisos de divergência de volume na capa ──────────────────────────────
    avisos_capa = ""
    if snapshots_raw and len(snapshots_raw) > 1:
        avisos = detectar_divergencias_sku(snapshots_raw)
        if avisos:
            linhas_aviso = r"\\" + chr(10) + "  ".join(avisos)
            avisos_capa = (
                "\n  \\vspace{0.5cm}\n"
                "  \\begin{mdframed}[linecolor=BrickRed, linewidth=1.5pt,\n"
                "    backgroundcolor={red!5}, innertopmargin=8pt, innerbottommargin=8pt,\n"
                "    innerleftmargin=10pt, innerrightmargin=10pt]\n"
                "  {\\color{BrickRed}\\textbf{{\\large $\\blacktriangle$\\ "
                "Aten\\c{c}\\~ao --- Diverg\\^encia de volume de SKUs}}}\\par\\smallskip\n"
                "  {\\small As seguintes marcas apresentam diferen\\c{c}a relevante "
                "de SKUs entre per\\'{{\\i}}odos,\\\\\n"
                "  o que pode indicar coleta incompleta. Verifique antes de "
                "interpretar os ajustes de pre\\c{c}o:}\\par\\smallskip\n"
                "  {\\small " + linhas_aviso + "}\n"
                "  \\end{mdframed}"
            )
...
def main():
    ...
    if args.export:
        ...
        # Montar lista de snapshots brutos para comparação de volume na capa
        # Inclui todos os snapshots carregados (hoje + periodos anteriores)
        snapshots_raw = [{"label": base_date.strftime("%d/%m/%Y"), "df": df_hoje}]
        if df_s1 is not None:
            label_s1_raw = data_semana1.strftime("%d/%m/%Y") if data_semana1 else "semana 1"
            snapshots_raw.append({"label": label_s1_raw, "df": df_s1})
        if df_mom is not None:
            snapshots_raw.append({"label": data_mom.strftime("%d/%m/%Y"), "df": df_mom})
