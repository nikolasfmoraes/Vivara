...
def _fmt_adj(val: str) -> str:
    """Colore positivo/negativo no LaTeX e escapa o % para LaTeX."""
    if val == "-":
    ...
    try:
        ...
        if n > 0:
            return r"\textcolor{red}{" + val_tex + r"}"
        elif n < 0:
            return r"\textcolor{teal}{" + val_tex + r"}"
        else:
            return val_tex
    except:
...
def gerar_latex(
    ...
    df_mix: pd.DataFrame,
    periodos: list[dict],   # [{"label": str, "df_band": df, "df_aj": df}, ...]
) -> str:
    """
    ...
    for per in periodos:
        ...
        aj_header = " & ".join([f"\\textbf{{{latex_escape(m)}}}\\\\ Avg & Wtd" for m in marcas_aj])
        ...
        marcas_header_aj = " & ".join(
            [f"\\multicolumn{{2}}{{c}}{{{latex_escape(m)}}}" for m in marcas_aj]
        )
        sub_header_aj = " & ".join(["Avg.adj & Wtd.adj"] * n_marcas_aj)
...
\toprule
\textbf{{Dimensão}} & {marcas_header_aj} \\
 & {sub_header_aj} \\
\midrule
{aj_rows}\bottomrule
\end{{tabular}}
...
Positivo indica alto ticket; permite comparar posicionamento entre marcas.
...
\textcolor{{red}}{{Vermelho}} = alta. \textcolor{{teal}}{{Verde}} = queda.
{sem2_aviso}
{secao2_blocks}
...
def main():
    ...
    if args.export:
        ...
        latex_path = REPORTS_DIR / f"{base_str}_relatorio.tex"
        with open(latex_path, "w", encoding="utf-8") as f:
            f.write(latex_str)
        log.info(f"  LaTeX gerado: {latex_path}")

        # ── Compilar PDF automaticamente ──
        # Roda pdflatex duas vezes: primeira gera estrutura, segunda resolve
        # referências internas (índice, links). Output vai para REPORTS_DIR.
        ...
        if pdflatex is None:
            ...
            pdf_path = REPORTS_DIR / f"{base_str}_relatorio.pdf"
            # Rodar com cwd=REPORTS_DIR e passar só o nome do arquivo
            # para evitar problema de path relativo vs absoluto no pdflatex
            cmd = [
                pdflatex,
                "-interaction=nonstopmode",
                latex_path.name,   # só o nome do arquivo, sem o diretório
            ]
            for rodada in [1, 2]:
                result = subprocess.run(
                    cmd,
                    capture_output=True,
                    cwd=str(REPORTS_DIR.resolve()),
                )
                # pdflatex pode retornar latin1 em alguns sistemas
                stdout_text = result.stdout.decode("utf-8", errors="replace")
                if result.returncode != 0 and rodada == 2:
                    log.error(f"  Erro ao compilar PDF (codigo {result.returncode})")
                    log.error("  Ultimas linhas do log:\n" +
                              "\n".join(stdout_text.splitlines()[-15:]))
                else:
                    log.info(f"  Passagem {rodada}/2 concluida")

            # Limpar arquivos auxiliares gerados pelo pdflatex
            for ext in [".aux", ".log", ".toc", ".out"]:
                aux = REPORTS_DIR / f"{base_str}_relatorio{ext}"
                if aux.exists():
                    aux.unlink()
            ...
            if pdf_path.exists():
                ...
                log.warning("  PDF nao foi gerado — verifique erros acima.")
